...
//  uint8_t randomnumber[35];
  ...
  //uint8_t ret_code = sha204.sha204e_configure_diversify_key();
  ...
  uint8_t ret_code_random_number = sha204.sha204m_random(command, response_random, RANDOM_SEED_UPDATE);
  ...
  memset(command, 0, sizeof(command));
  memset(response, 0, sizeof(response)); 
  ret_code_nonce = sha204.sha204m_nonce(command, response, NONCE_MODE_PASSTHROUGH, randomnumber);
  if (ret_code_nonce != SHA204_SUCCESS) {
    //sha204p_sleep();
    return ret_code_nonce;
  }
  ...
  memset(command, 0, sizeof(command));
  memset(response, 0, sizeof(response));
  uint8_t ret_code_gen_dig = sha204.sha204m_gen_dig(command, response, GENDIG_ZONE_DATA, SHA204_KEY_PARENT, command_derive_key);
  ...
  memset(command, 0, sizeof(command));
  memset(response, 0, sizeof(response));
  ret_code_mac = sha204_1.sha204m_mac(command, response_mac, MAC_MODE_CHALLENGE, SHA204_KEY_CHILD, randomnumber);
  if (ret_code_mac != SHA204_SUCCESS) {
  	//sha204e_sleep();
  	return ret_code_mac;
  }
